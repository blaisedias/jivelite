...
const char* delims = " :\t\n";
// keep in sequence with MEMUSAGE enum
const char* MEMUSAGE_TOKENS[] = {
    "MemTotal",
    "MemFree",
    "MemAvailable",
    "SwapCached",
...
// keep in sequence with MEMUSAGE_TOKENS
enum MEMUSAGE {
    MemTotal,
    MemFree,
    MemAvailable,
...
void process_mu_token(char* token, ptr_mem_usage pmu) {
    int i;
    for(i = 0; i < sizeof(MEMUSAGE_TOKENS)/sizeof(*MEMUSAGE_TOKENS); i++) {
        if (!strcmp(MEMUSAGE_TOKENS[i], token)) {
            char* strvalue = strtok(NULL, delims);
// ignore unit assume KiB for now.            
//            char* unit = strtok(NULL, delims);
            long value = strtol(strvalue, NULL, 10);
            // convert values to MiB
            switch(i) {
                case MemTotal:
                    pmu->MemTotal = (int)(value/1024);
                    break;
                case MemFree:
                    pmu->MemFree = (int)(value/1024);
                    break;
                case MemAvailable:
                    pmu->MemAvailable = (int)(value/1024);
                    break;
                case SwapCached:
                    pmu->SwapCached = (int)(value/1024);
                    break;
                case SwapTotal:
                    pmu->SwapTotal = (int)(value/1024);
                    break;
                case SwapFree:
                    pmu->SwapFree = (int)(value/1024);
                    break;
            }
        }
    }
