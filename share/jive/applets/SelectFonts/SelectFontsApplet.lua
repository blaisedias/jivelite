...
function selectFonts(self, title)
	...
	local currentFontExists = false
	log:debug("current ", current)
	for name, _ in pairs(fontsdb) do
		currentFontExists = current == name
		if currentFontExists then
			break
		end
	end

	if not currentFontExists then
		-- select the default typeface is none is selected or the selected
